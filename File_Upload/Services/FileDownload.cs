...
namespace File_Upload.Services
{
    ...
    public class FileDownload : IFileDownload
    {
        ...
        public FileDownload(IWebHostEnvironment web,IJSRuntime runtime)
        {
            _web = web;
            _runtime = runtime;
        }
        ...
        {
            await _runtime.InvokeVoidAsync("downloadFile", url);
        }

        public async Task<List<string>> GetUploadedFiles()
        {
            var base64Urls = new List<string>();
            var uploadPath = Path.Combine(_web.WebRootPath, "uploads");
